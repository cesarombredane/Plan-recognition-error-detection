...
extendedPlanLibrary::extendedPlanLibrary(const char* spl) {
    ...
    if (tst > 0)
        ...
        cout << "Parsing errors detected, status = " << tst << endl << "stopped at: " << tail << endl;
...
extendedPlanLibrary::extendedPlanLibrary(float noisePRCT, int goal, int _size, int depth, int widthAND, int widthOR) { // generate a generic plan library for tests
